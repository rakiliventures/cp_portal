...
| A4 | Attendance Module: admin creates event (title, date, venue); for existing event, view attendees and add attendee by searching members or adding guest (full name); list shows Member or Guest per attendee | §4.3 Attendance Tracking; USER_FLOWS §1.3–1.4 | Add-by-name (member search or guest); each row labeled Member/Guest |
...
| — | Roles/modules with granular permissions (view, edit, create, delete); Treasurer = Finance full, Moderator = Finance read-only; default member = Personal Dashboard, Group Wide Reports, Past Events, Calendar (view only); temporary Events (create/edit/delete); User Management module | §3 Roles, Modules & Granular Permissions; §4.9 User Management; §5.1 Module, UserModuleAssignment | USER_FLOWS §1.8 |
