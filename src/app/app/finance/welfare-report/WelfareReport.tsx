...
export function WelfareReport({ year, availableYears, balanceBF, payments, expenses }: Props) {
  ...
  return (
    <>
      ...
      <div className="no-print mb-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <div className={`rounded-xl border px-4 py-3 ${cashflow >= 0 ? "border-blue-200 bg-blue-50" : "border-red-200 bg-red-50"}`}>
          <p className="text-[11px] font-medium uppercase tracking-wide text-slate-500">Cashflow at Hand</p>
          <p className={`mt-1 text-lg font-bold tabular-nums leading-none ${cashflow >= 0 ? "text-blue-700" : "text-red-600"}`}>
          ...
          <table className="w-full min-w-[600px] text-sm">
            ...
            <tbody>
              {ledger.map((row) => {
                ...
                if (row.kind === "payment") {
                  return (
                    <tr key={`pay-${row.month}`} className="border-b border-slate-100 bg-blue-50/40 hover:bg-blue-50">
                      <td className="px-4 py-3 text-slate-600 whitespace-nowrap">{MONTH_NAMES[row.month - 1]} {year}</td>
                      <td className="px-4 py-3 font-medium text-slate-700">
                        {MONTH_NAMES[row.month - 1]} Welfare Contributions
                      </td>
                ...
                return (
                  <tr key="total" className="border-t-2 border-slate-300 bg-slate-100">
                    ...
                    <td className={`px-4 py-3 text-right font-bold tabular-nums ${row.balance >= 0 ? "text-blue-700" : "text-red-600"}`}>
                      {row.balance < 0 ? "−" : ""}{fmt(row.balance)}
                    </td>
                  </tr>
                );
      ...
      <div className="hidden print:block mt-6 text-xs text-slate-400">
        Welfare Kitty Summary Report · {year} · All amounts in KES
      </div>
    </>
  );
}
