...
import { WelfareReport, type MonthPayment, type ExpenseEntry } from "./WelfareReport";
...
export default async function WelfareReportPage({
  searchParams,
}: {
  searchParams: Promise<{ year?: string }>;
}) {
  const session = await getServerSession(authOptions);
  if (!session?.user) redirect("/login?callbackUrl=/app/finance/welfare-report");
  ...
  const welfareAccount = await prisma.paymentAccount.findFirst({
    where:  { code: "CP-WELFARE" },
    select: { id: true },
  });
  ...
  const [
    ...
    welfareAccount
      ? prisma.payment.findMany({
          where:   { accountId: welfareAccount.id, datePaid: { gte: yearStart, lt: yearEnd } },
    ...
    welfareAccount
      ? prisma.payment.aggregate({
          where: { accountId: welfareAccount.id, datePaid: { lt: yearStart } },
          _sum:  { amount: true },
        })
      : Promise.resolve({ _sum: { amount: null } }),

    prisma.expense.aggregate({
      where: { categoryOrAccount: "CP-WELFARE", expenseDate: { lt: yearStart } },
      _sum:  { amount: true },
    }),

    welfareAccount
      ? prisma.payment.findFirst({
          where:   { accountId: welfareAccount.id },
    ...
    prisma.expense.findFirst({
      where:   { categoryOrAccount: "CP-WELFARE" },
      orderBy: { expenseDate: "asc" },
  ...
  return (
    <div className="min-w-0">
      <div className="mb-5">
        <h1 className="page-heading">Welfare Summary Report</h1>
        <p className="text-sm text-slate-500">
          Monthly payments collected and individual expenses charged to Welfare Kitty · {selectedYear}
        </p>
      </div>
